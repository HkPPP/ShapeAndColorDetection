import cv2
import imutils
from shape_and_color_detection.Classification.classify_shapes_and_colors import iden_shapes
...
def find_contour(thresholded_image):
    ...
    # Multiply by ratio to scale contours back to the scale of the original image
    cnts = cnts * ratio
    return cnts
...
def black_and_white(image, color_space="BGR"):
    ...
    # Threshold image to blacken or whiten pixels
    thresh = cv2.threshold(blurred, 60, 255, cv2.THRESH_BINARY)[1]

    # Invert threshold if the background is white
